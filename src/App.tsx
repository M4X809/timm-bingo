	...
	persist(
		(set, get) => ({
			...
			toggleElement: (index: number) =>
				set({ currentGrid: get().currentGrid.map((el, i) => (i === index ? { ...el, checked: !el.checked } : el)) }),
			threeInARow: false,
		...
		}),
		{
			...
			partialize: (state) => ({
				...
				threeInARow: state.threeInARow,
				bingoCount: state.bingoCount,
				currentBingo: state.currentBingo,
			}),
		},
	),
);
	...
	useEffect(() => {
		if (threeInARow && !currentBingo) {
			...
			modals.open({
				...
				children: (
					<>
						<Text>Du hast 3 in einer Reihe und somit gewonnen!</Text>
						<Button
							mt={15}
							fullWidth
							variant="gradient"
		...
		<>
			<Group justify="center" pt={25}>
				<Button
					variant="gradient"
					gradient={{ from: "red", to: "orange" }}
					leftSection={<FontAwesomeIcon icon={faWarning} fontSize={20} />}
					onClick={() => {
				...
				<Text c={"dimmed"}>Bingo Count: {bingoCount}</Text>
			</Group>
